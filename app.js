...
app.get("/players/:playerId/matches", async (request, response) => {
  ...
  const getPlayerMatchesDetails = `
        ...
        WHERE player_id = ${playerId};
    `;
  const playerMatchDetails = await db.all(getPlayerMatchesDetails);
  response.send(playerMatchDetails);
});

app.get("/matches/:matchId/players", async (request, response) => {
  const { matchId } = request.params;
  const getPlayersDetailsForMatch = `
        SELECT player_id AS playerId,
        player_name AS playerName
    FROM 
    ...
    `;
  const playersDetailsList = await db.all(getPlayersDetailsForMatch);
...
app.get("/players/:playerId/playerScores", async (request, response) => {
  ...
  const getPlayersScores = `
    ...
    `;
  const playerScores = await db.all(getPlayersScores);
